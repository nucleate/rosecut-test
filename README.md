...
Create `env.json` file with similar structure to `env.sample.json`. Or you can use command:

```sh
cp -n env.sample.json env.json
```

Start your DynamoDB instance locally using Docker:
...
```bash
aws cloudformation delete-stack --stack-name rosecut-test
```
